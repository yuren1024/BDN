...
def circle_filter_high_f(fshift, radius_ratio,img):
    """
    过滤掉除了中心区域外的高频信息
    """
...
def circle_filter_low_f(fshift, radius_ratio,img):
    """
    去除中心区域低频信息
    """
...
def get_low_high_f(img, radius_ratio=0.1, D=50):
    ...
    # 获取低频和高频部分
    hight_parts_fshift = circle_filter_low_f(fshift.copy(), radius_ratio=radius_ratio,img = img)  # 过滤掉中心低频
    low_parts_fshift = circle_filter_high_f(fshift.copy(), radius_ratio=radius_ratio,img = img)
